...
1.  **Maintain Physics Scales & Collision Bitmasks**:
    ...
    *   Always respect collision category layers defined in `src/core/Constants.ts`:
        *   `DEFAULT` (0x0001)
        *   `TERRAIN` (0x0002): collides with shards, tools, buildings.
        *   `SHARDS` (0x0004): collides with terrain, shards, tools, buildings.
        *   `BUILDINGS` (0x0008): collides with shards.
        *   `TOOLS` (0x0010): collides with terrain, shards, tools, buildings.
...
4.  **Save System Serialization**:
    *   When adding new entity types (e.g. new structures, particles, or items), ensure they are correctly integrated into both `serializeState()` and `deserializeState()` inside `src/save/SaveLoadController.ts` to preserve save compatibility.
