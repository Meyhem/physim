...
*   If no active task is running, call `run_command` with the following parameters:
    ...
    *   `WaitMsBeforeAsync`: `3000` (gives the server time to spin up and bind)

### 3. Verify Server Logs & Expose URL
*   Once the task starts, check the log file using `manage_task` with `Action: "status"` for the task ID returned.
*   Extract the local address (default is `http://localhost:5173/`).
*   Output the clickable link to the user.
